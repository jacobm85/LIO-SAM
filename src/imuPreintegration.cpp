...
public:
    ...
    std::string odomImuChildFrame;

    IMUPreintegration(const rclcpp::NodeOptions & options) :
            ParamServer("lio_sam_imu_preintegration", options)
    {
        // Declare and get odom child frame param (defaults to "odom_imu")
        odomImuChildFrame = declare_parameter<std::string>("odomImuChildFrame", "odom_imu");

        // ... (rest of constructor as in your version, unchanged) ...
    }
    ...
    void imuHandler(const sensor_msgs::msg::Imu::SharedPtr imu_raw)
    {
        ...
        odometry.child_frame_id = odomImuChildFrame; // <-- now parameterized
